						...
						<div class="card-tools">
							<div class="btn-group">
								...
								<div class="dropdown-menu">
									<a class="dropdown-item" href="{{ route('backend.user.export') }}"><i class="far fa-file-excel"></i> Excel</a>
								</div>
							</div>
							<a class="btn btn-sm btn-success" href="{{ route('backend.user.create') }}"><i class="fas fa-plus"></i> Add User</a>
						</div>
					</div>
					<!-- /.card-header -->
					<div class="card-body">
							...
							<div class="form-row mb-4">
								<div class="col-12 col-md-4">
									...
									<select class="w-75 filter-form select2" id="filter_role">
									  	...
									  	@foreach(\App\Model\Backend\Role::hasUser()->get() as $role)
								...
								<div class="col-12 col-md-4">
									...
									<select class="form-control d-inline-block w-75 filter-form" id="filter_status">
									  	<option value="">ทั้งหมด</option>
									  	@foreach(\App\Model\User::statusOptions() as $value => $text)
									  		<option value="{{ $value }}">{{ $text }}</option>
									  	@endforeach
									</select>
								</div>
							</div>
							<table class="table table-bordered table-striped table-hover table-f-small w-100" id="user_datatable">
...
@push('after-scripts')
	<script>
	    ...
	    var dtID = '#user_datatable';
	    var dtTable = $(dtID).DataTable({
	        processing: true,
	        serverSide: true,
	        responsive: true,
	        ajax: {
	            url: "{{ route('backend.user.list-data') }}",
	            data: function(d) {
