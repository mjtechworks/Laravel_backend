...
<section class="content">
	<div class="container-fluid">
		<div class="row">
			<div class="col">
				<form action="{{ route('backend.{{ model }}.store') }}" method="POST" id="form_create_{{ model }}" autocomplete="off" enctype="multipart/form-data">
					@csrf
					<div class="card card-success card-outline">
						<div class="card-header">
							<h3 class="card-title py-1"><i class="fas fa-plus"></i> เพิ่ม {{ model }}</h3>
...
@push('after-scripts')
<script>
	$('#form_create_{{ model }}').validate({
		rules: {
			name: 'required',
			email: {
				required: true,
				email: true,
				remote: '{{ route('backend.{{ model }}.check-valid-email') }}'
			},
			password: {
				required: true,
				minlength: 8
